...
public class ReadData {
    ...
    public static void main(String[] args) {
        ...
        sessionImplementor.getTransaction().begin();
        for (Person person : persons) {
            if (person != null) {
                sessionImplementor.persist(person);
            }
        }
        sessionImplementor.getTransaction().commit();
        
        
        Person person = (Person) sessionImplementor.get(Person.class, 2);
        if (person != null) {
